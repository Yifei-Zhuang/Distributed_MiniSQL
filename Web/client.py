...
class Client:
    ...
    def get_buffer_str(self):
        result = ""
        result += "TABLE NAME -> REGION NAME\n"
        for table in self.buffer.table_region_map:
            result += table + " -> " + self.buffer.table_region_map[table] + "\n"
            
        result += "\nREGION NAME -> HOST\n"
        for region in self.buffer.region_hosts_map:
            result += region + " -> " + self.buffer.region_hosts_map[region] + "\n"
        return result
    ...
    def exec_non_print(self, sql):
        ...
        if self.is_create_sql(sql): # create命令,需要根据负载均衡找到所需的region_url
            ...
            table_name = self.get_table_from_sql(sql)
            if table_name == None:
                res_log += "No table in sql! CMD abort!" + "\n"
                return
            url = self.buffer.get_region_url(table_name)
            # 检查Buffer内是否含有该table
            if url == None:
                res_log += "INFO: table_name not found in buffer!" + "\n"
                res_log += "INFO: Buffer will be refreshed!" + "\n"
                self.buffer.refresh_buffer()
                url = self.buffer.get_region_url(table_name)
                if url == None:
                    res_log += "WARNING: table_name not found in refreshed buffer!" + "\n"
                    res_log += "WARNING: Please check your sql!" + "\n"
                    return
        
        res_log = res_log + "Table: " + table_name + "  Region: " + url + "\n"
        ...
        try:
            ...
            res_log += "ERROR: sql exec timeout!" + "\n"
            return None
        ...
        if response.ok:
            ...
            if result["statusCode"].lower() != "success":
                res_log += "WARNING: sql exec failed!"  + "\n"
                res_log += result["msg"]
            else:
                res_log += "Info: sql exec success!" + "\n"
                if result["type"].lower() != "select":
                    res = result["msg"]
                else:
                    res = self.select_print(result["msg"])
        else:
            res_log += "WARNING: response not OK!" + "\n"
            res_log = response.json()
    ...
    def select_print(self, msg: str):
        ...
        return tabulate(table_rows, headers= fields, tablefmt='presto').replace("+", "|")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if cmd.lower() == 'h' or cmd.lower() == 'help':
            ...
            print(client.exec_non_print(cmd))